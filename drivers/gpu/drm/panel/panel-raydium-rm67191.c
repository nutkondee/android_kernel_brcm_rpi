...
//ToDo: change this method to multiple raydium panel
...
static int rm67191_enable(struct rad_panel *panel)
{
	...
	ret = rad_panel_push_cmd_list(dsi,&mcs_rm67191[0],ARRAY_SIZE(mcs_rm67191));
	if (ret < 0) {
		dev_err(dev, "Failed to send MCS rm67191 (%d)\n", ret);
		goto fail;
	}
	...
	if (ret < 0) {
		dev_err(dev, "Failed to do Software Reset rm67191 (%d)\n", ret);
		goto fail;
	}
	...
	if (ret < 0) {
		dev_err(dev, "Failed to set DSI mode rm67191 (%d)\n", ret);
		goto fail;
	}
	/* Set tear ON */
	ret = mipi_dsi_dcs_set_tear_on(dsi, MIPI_DSI_DCS_TEAR_MODE_VBLANK);
	if (ret < 0) {
		dev_err(dev, "Failed to set tear ON rm67191 (%d)\n", ret);
		goto fail;
	}
	/* Set tear scanline */
	ret = mipi_dsi_dcs_set_tear_scanline(dsi, 0x380);
	if (ret < 0) {
		dev_err(dev, "Failed to set tear scanline rm67191 (%d)\n", ret);
		goto fail;
	}
	/* Set pixel format */
	ret = mipi_dsi_dcs_set_pixel_format(dsi, color_format);
	dev_dbg(dev, "Interface rm67191 color format set to 0x%x\n", color_format);
	if (ret < 0) {
		dev_err(dev, "Failed rm67191 to set pixel format (%d)\n", ret);
		goto fail;
	}
	/* Exit sleep mode */
	ret = mipi_dsi_dcs_exit_sleep_mode(dsi);
	if (ret < 0) {
		dev_err(dev, "Failed to exit sleep mode rm67191 (%d)\n", ret);
		goto fail;
	}
	...
	if (ret < 0) {
		dev_err(dev, "Failed to set display ON rm67191 (%d)\n", ret);
		goto fail;
	}
...
static int rm67199_enable(struct rad_panel *panel)
{
	...
	ret = rad_panel_push_cmd_list(dsi,&mcs_rm67199[0],ARRAY_SIZE(mcs_rm67199));
	if (ret < 0) {
		dev_err(dev, "Failed to send MCS rm67199 (%d)\n", ret);
		goto fail;
	}
	...
	ret = mipi_dsi_generic_write(dsi, (u8[]){ 0xC2, dsi_mode }, 2);
	if (ret < 0) {
		dev_err(dev, "Failed to set DSI mode rm67199 (%d)\n", ret);
		goto fail;
	}
	...
	ret = mipi_dsi_dcs_set_tear_on(dsi, MIPI_DSI_DCS_TEAR_MODE_VBLANK);
	if (ret < 0) {
		dev_err(dev, "Failed to set tear ON rm67199 (%d)\n", ret);
		goto fail;
	}
	...
	if (ret < 0) {
		dev_err(dev, "Failed to set tear scanline rm67199 (%d)\n", ret);
		goto fail;
	}

	/* Set pixel format */
	ret = mipi_dsi_dcs_set_pixel_format(dsi, color_format);
	dev_dbg(dev, "Interface color rm67199 format set to 0x%x\n",
			     color_format);
	if (ret < 0) {
		dev_err(dev, "Failed rm67199 to set pixel format (%d)\n", ret);
		goto fail;
	}
	...
	ret = mipi_dsi_dcs_exit_sleep_mode(dsi);
	if (ret < 0) {
		dev_err(dev, "Failed rm67199 to exit sleep mode (%d)\n", ret);
		goto fail;
	}
	...
	*/
	...
	ret = mipi_dsi_dcs_set_display_on(dsi);
	if (ret < 0) {
		dev_err(dev, "Failed rm67199 to set display ON (%d)\n", ret);
		goto fail;
	}
